...
class TestAIUFamilyExpansion:
    ...
    def test_g_family_layer_affinity(self):
        ...
        for aiu in AIU_FAMILY["G_distributed"]:
            ...
            assert any(l in ["APP", "DOMAIN", "L4_application", "L3_domain"] for l in affinity), \
                f"{aiu} 应亲和应用层或领域层"
    ...
    def test_h_family_layer_affinity(self):
        ...
        for aiu in AIU_FAMILY["H_governance"]:
            affinity = AIU_LAYER_AFFINITY[aiu]
            assert any(l in ["PLATFORM", "CC", "DOMAIN", "L1_platform", "cross_cutting"] for l in affinity), \
                f"{aiu} 应亲和平台层/横切层"

    def test_i_family_layer_map(self):
        """I 族的主层级应该是平台层（L1_platform）"""
        for aiu in AIU_FAMILY["I_observability"]:
            assert AIU_LAYER_MAP[aiu] == "L1_platform", \
                f"{aiu} 主层级应为 L1_platform（可观测性属于平台横切能力）"

    def test_layer_names_are_valid(self):
        """所有 AIU_LAYER_MAP 中的层级名应使用 dag_model.py VALID_LAYERS 中的值"""
        # v4.0：统一使用 Clean Architecture 名（L1_platform 等），不再使用 DDD 名（PLATFORM 等）
        valid_layers = {
        ...
        for aiu_type, layer in AIU_LAYER_MAP.items():
            assert layer in valid_layers, \
                f"{aiu_type} 使用了无效层级名: {layer}（应为 VALID_LAYERS 中的值）"
