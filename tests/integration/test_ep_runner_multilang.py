...
def test_ep_runner_track_a_pipeline(fixture_name, request):
    ...
    with patch("mms.dag.dag_model._DAG_DIR", project_root / "docs" / "memory" / "_system" / "dag"), \
        ...
        result = pipeline.run(ep_id=ep_id, dry_run=True, model="qwen3-coder-plus", project_root=project_root)
...
def test_ep_runner_track_b_autonomous(isolated_python_project):
    ...
    with patch("mms.workflow.ep_runner._resolve_execution_track", return_value="autonomous"), \
         patch("mms.workflow.ep_runner.EpRunPipeline._run_autonomous_units") as mock_auto_run:
        ...
        result = pipeline.run(ep_id=ep_id, dry_run=True, model="claude-opus-4", project_root=project_root)
...
def test_ep_runner_resume_from_unit(isolated_python_project):
    ...
    with patch("mms.dag.dag_model._DAG_DIR", project_root / "docs" / "memory" / "_system" / "dag"), \
        ...
        # 指定从 U2 开始跑
        result = pipeline.run(ep_id=ep_id, dry_run=True, model="qwen3-coder-plus", from_unit="U2", project_root=project_root)
