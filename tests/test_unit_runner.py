...
class TestUnitRunnerDryRun:
    ...
    def test_dry_run_returns_changed_files(self, tmp_path):
        ...
        with _patch_dag_dir(dag_dir), \
            ...
            runner = UnitRunner()
            result = runner.run("EP-TEST", "U1", dry_run=True, project_root=tmp_path)
...
class TestUnitRunnerSuccess:
    ...
    def test_success_path(self, tmp_path):
        ...
        with _patch_dag_dir(dag_dir), \
            ...
            runner = UnitRunner()
            result = runner.run("EP-TEST", "U1", model="capable", project_root=tmp_path)
    ...
    def test_already_done_unit_skipped(self, tmp_path):
        ...
        with _patch_dag_dir(dag_dir), \
             patch("mms.execution.unit_runner._ROOT", tmp_path):
            runner = UnitRunner()
            result = runner.run("EP-TEST", "U1", project_root=tmp_path)
...
class TestUnitRunnerFailure:
    ...
    def test_unit_not_found_returns_failure(self, tmp_path):
        ...
        with _patch_dag_dir(dag_dir), \
             patch("mms.execution.unit_runner._ROOT", tmp_path):
            runner = UnitRunner()
            result = runner.run("EP-TEST", "U999", project_root=tmp_path)
    ...
    def test_missing_dag_returns_failure(self, tmp_path):
        ...
        with _patch_dag_dir(empty_dag_dir), \
             patch("mms.execution.unit_runner._ROOT", tmp_path):
            runner = UnitRunner()
            result = runner.run("EP-NOTEXIST", "U1", project_root=tmp_path)
...
class TestBatchRunner:
    ...
    def test_run_next_no_dag_returns_empty(self, tmp_path):
        ...
        with _patch_dag_dir(empty_dag_dir), \
             patch("mms.execution.unit_runner._ROOT", tmp_path):
            runner = BatchRunner()
            results = runner.run_next("EP-NOTEXIST", project_root=tmp_path)
        assert results == []
    ...
    def test_run_next_all_done_returns_empty(self, tmp_path):
        ...
        with _patch_dag_dir(dag_dir), \
             patch("mms.execution.unit_runner._ROOT", tmp_path):
            runner = BatchRunner()
            results = runner.run_next("EP-TEST", project_root=tmp_path)
        assert results == []
