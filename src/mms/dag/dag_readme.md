...
> **最后更新**：2026-05-04（反映 3-Batch 架构修复：filelock 必选、RBO-YAML OCP、A3 图谱连通性）
...
按 `exec_order` 排序后分配顺序 `aiu_id`（如 `aiu_1`, `aiu_2`...），并统一清空所有步骤的 `depends_on=[]`。

**执行顺序的表达方式**：

| 机制          | 用途                               |
| ----------- | -------------------------------- |
| `exec_order` | **唯一排序依据**（数字越小越先执行）              |
| `depends_on` | **始终为 `[]`**（不再作为调度依赖，避免 LLM 幻觉） |

> **设计理由**：`depends_on` 字段移除了"BSP 全同步屏障"和"LLM 稀疏依赖保留"两种逻辑，因为：（1）AIUStep 是执行提示层，不参与实际调度；（2）LLM 声明的依赖容易引入幻觉依赖；（3）`exec_order` 已足以表达顺序语义。
...
4. **统一 Feedback 入口**：`record_unit_feedback()` 和 `get_max_feedback_level()` 替代 `unit_runner` 中的直接文件读写
