...
#### 3.1 `tests/dag/test_task_decomposer.py`
...
#### 3.2 `tests/dag/test_aiu_cost_estimator.py`
...
#### 3.3 `tests/dag/test_atomicity_check_full.py`

**目标**：补充 A1/A2 检查器的覆盖（当前 test_cost_and_atomicity 只覆盖 A3）
...
#### 3.4 `tests/dag/test_aiu_registry_ocp_extended.py`
...
#### 3.5 `tests/dag/test_feedback_suggest.py`
...
| 模块 | 当前覆盖 | 目标 | 主要未覆盖路径 |
|------|---------|------|----------------|
| `atomicity_check.py` | ~70% | 90% | A1/A2 完整路径，图不可用降级 |
| `aiu_cost_estimator.py` | ~60% | 85% | 多文件计算，层因子分支 |
| `aiu_registry.py` | ~75% | 90% | custom/ 子目录，格式容错 |
| `task_decomposer.py` | ~40% | 80% | LLM 路径，解析容错 |
| `aiu_feedback.py` | ~65% | 85% | suggest() 全路径，unit_feedback |
...
- ✅ **R-old1：A3 假阳性**（新文件悖论）— 已修复 + 测试
- ✅ **R-old2：CBO 毒性正反馈**（history_factor 无上限）— 算法已验证
- ✅ **R-old3：FileLock 跨进程竞争**— 多进程测试已验证
- ✅ **R-old4：RBO 规则硬编码**（OCP 违反）— YAML 驱动已测试
