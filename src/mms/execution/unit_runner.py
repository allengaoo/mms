...
用法：
    from mms.execution.unit_runner import UnitRunner, RunResult
    runner = UnitRunner()
...
def _generate_unit_context(unit, model: str = "capable") -> str:
    ...
    try:
        from mms.execution.unit_context import generate_unit_context  # type: ignore[import]
    except ImportError:
        try:
            from mms.execution.unit_context import generate_unit_context  # type: ignore[import]
        except ImportError:
...
def _aiu_feedback_analysis(
    ...
    try:
        sys.path.insert(0, str(_HERE))
        from mms.dag.aiu_types import classify_error, AIUErrorPattern, ERROR_TO_FEEDBACK_LEVEL  # type: ignore[import]
    except ImportError:
...
def _suggest_prerequisite_aiu(error_pattern, error_msg: str) -> str:
    ...
    try:
        from mms.dag.aiu_types import AIUErrorPattern, AIUType  # type: ignore[import]
    except ImportError:
...
class UnitRunner:
    ...
    def run(
        ...
        for attempt in range(1, self.max_retries + 2):  # 1, 2, 3
            ...
            try:
                from mms.execution.file_applier import parse_and_validate  # type: ignore[import]
            except ImportError:
                from mms.execution.file_applier import parse_and_validate  # type: ignore[import]
            ...
            if arch_ok and test_ok:
                ...
                try:
                    from mms.trace.collector import get_tracer  # type: ignore[import]
                    _tracer = get_tracer(ep_id)
                    if _tracer:
        ...
        if feedback_result.get("action") == "retry_with_expanded_budget":
            ...
            if expanded_context and expanded_context != unit_context:
                ...
                if raw_l1:
                    try:
                        from mms.execution.file_applier import parse_and_validate, FileApplier  # type: ignore[import]
                        from mms.execution.sandbox import GitSandbox  # type: ignore[import]
                    except ImportError:
                        from mms.execution.file_applier import parse_and_validate, FileApplier  # type: ignore[import]
                        from mms.execution.sandbox import GitSandbox  # type: ignore[import]
