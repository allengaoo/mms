...
④ 织造（代码生成）    qwen3-coder-next 生成 Diff，受模板骨架约束
      ...
      │
      ▼
⑤ 验布（自动验证）    postcheck：pytest 验证 + AST 契约检查 + DB 迁移对齐 + 记忆新鲜度检测
...
| EP 全自动 Pipeline | ✅ 稳定 | `mulan ep run`：precheck → units → postcheck，支持无人值守 |
...
```
mulan bootstrap  →  扫描 AST  →  在本体 YAML 中填充 ast_pointer.fingerprint
mulan postcheck  →  ast_diff  →  fingerprint 变更 → ontology_syncer 标记 drift=true
...
```bash
mulan ep run EP-NNN                      # 自动执行，遇到关键决策点暂停确认
...
通过 `mulan seed ingest` 可从 GitHub 吸收外部规范自动生成新种子包（见 [Rule Absorber](#rule-absorber)）。
...
mulan unit compare --ep EP-NNN --unit U1                 # 生成机械 diff + qwen3-32b 语义评审报告
...
```bash
mulan search kafka replication           # 关键词检索（Jaccard，无向量）；章节入口预匹配亦可触发全文检索以降低 token 消耗
...
mulan private promote EP-NNN note.md L3_domain MEM-L-028  # 升级为公有记忆
```
...
```bash
mulan status                            # Provider 健康 + 熔断器 + 记忆统计 + 记忆图健康
...
mulan verify                            # 全面健康检查（schema/index/docs/frontend）
mulan reset-circuit                     # 重置所有熔断器
```

`mulan status` 包含记忆图健康报告输出示例：
...
- ✅ **图健康监控**：`mulan status` 实时显示节点分布、边覆盖率、孤立率、图密度
...
**自学习层**
- ✅ **Rule Absorber**：`mulan seed ingest <url>` 将外部规范蒸馏为 MMS 种子包
...
- **VSCode / Cursor 插件**：将 `mulan inject` 和 `mulan status` 集成进 IDE 侧边栏，图形化操作

**增强知识共享**
- **跨项目本体迁移**：`mulan export-ontology` 和 `mulan import-ontology`，支持在多个项目间共享领域知识
...
mulan validate --changed-only                        # 记忆文件 Schema 校验（如有修改）
...
- **新增 CLI 命令**：同步更新 `_COMMAND_DOCS` 字典（用于 `mulan help`）和本文件的 CLI 参考章节
...
```bash
# 验证 LLM 环境
mulan status                            # 检查百炼 API + 记忆系统状态

# 开启诊断追踪调试新功能
mulan trace enable EP-DEV --level 8
# ... 运行你的功能 ...
mulan trace show EP-DEV
