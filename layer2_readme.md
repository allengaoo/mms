...
| | `graph_resolver.py` | 59% | 中等 |
| | `dream.py` | 62% | 中等 |
| | `freshness_checker.py` | 66% | 中等 |
| | `injector.py` | 0% | ❌ 待补全 |
| | `intent_classifier.py` | 0% | ❌ 待补全 |
| | `repo_map.py` | 0% | ❌ 待补全 |
| | `codemap.py` | 0% | ❌ 待补全 |
| | `entropy_scan.py` | 0% | ❌ 待补全 |
| | `task_matcher.py` | 0% | ❌ 待补全 |
| | `memory_actions.py` | 0% | ❌ 待补全 |
| **Layer 2 合计** | — | **49%** | 中等 |
...
本轮新增以下测试文件（均受 `.gitignore` 管理，不入库但在本地运行）：
...
Memory Engine 是最大缺口，7 个核心文件为 0%，主要原因是这些模块依赖 LLM 调用或复杂的文件系统状态，需要大量 mock 工作。建议补全优先级：

1. **`injector.py`（0%）**：Layer 1→Layer 2 的核心接口，高优先级
2. **`intent_classifier.py`（0%）**：影响检索质量
3. **`repo_map.py`（0%）**：被多个组件依赖的基础工具
4. **`graph_resolver.py`（59%）**：核心图算法，已有基础，完善高优先
